...
-(void)zaman{
    ...
    j=rand()%500;
    hareket=[CCActionMoveTo actionWithDuration:2 position:ccp(renk2.position.x, j)];
    [renk2 runAction:hareket];
    
    j=rand()%500;
    hareket=[CCActionMoveTo actionWithDuration:2 position:ccp(renk3.position.x, j)];
...
-(BOOL)ccPhysicsCollisionBegin:(CCPhysicsCollisionPair *)pair Top:(CCNode *)_top zemin:(CCNode *)zeminPar
{
    ...
    [[CCDirector sharedDirector] replaceScene:[CCBReader loadAsScene:@"Level2"] withTransition:[CCTransition transitionFadeWithDuration:2.0]];
...
- (void)touchBegan:(CCTouch *)touch withEvent:(CCTouchEvent *)event
{
    ...
    else if(touchLocation.x>=180.0 && (_gemi.position.x<200))
    {
        CCAction* hareketEtme=[CCActionMoveTo actionWithDuration:0.2 position:ccp(_gemi.position.x+30,_gemi.position.y)];
